...
if (redirectUrl) {
    ...
    // const roomId = document.getAttribute('data-room-id'); // 선택된 채팅방의 ID 가져오기
    // const receiveUserId = document.getAttribute('data-receive-user-id'); // 선택된 채팅방의 수신자 ID 가져오기
    // const receiveUserName = document.getAttribute('data-receive-user-name'); // 선택된 채팅방의 수신자 이름 가져오기
    ...
    // 두 번째 AJAX 요청: 채팅 메시지 데이터 가져오기
    // fetch(redirectUrl)
    // .then(response => response.json())
    // .then(data => {
    //     console.log(`데이터 가져옴 ${data.room_id}, ${data.receive_user_name}`);
